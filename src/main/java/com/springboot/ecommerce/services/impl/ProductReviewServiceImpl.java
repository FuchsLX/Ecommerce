...
public class ProductReviewServiceImpl implements ProductReviewService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(ProductReviewServiceImpl.class);

  @Override
  public List<ProductReviewDTO> getAllReviews(String productId, int pageNo) {
    return this
        .getAllReviewWithPagination(productId, pageNo, 1, "created_at", "desc")
        .stream()
        .map(pr
             -> ProductReviewDTO.builder()
                    ...
                    .customerName(pr.getCustomer().getFirstName() + " " +
                                  pr.getCustomer().getLastName())
                    .build())
  ...
  public ProductReviewDTO save(ProductReviewDTO productReviewDTO) {
    if (productReviewDTO.getId() == null)
      return this.createNewReview(productReviewDTO);
    else
      return this.updateReview(productReviewDTO);
  }

  @Transactional
  protected ProductReviewDTO
  createNewReview(ProductReviewDTO productReviewDTO) {
    var customer =
        userRepository.findByEmail(productReviewDTO.getCustomerEmail())
            .orElse(null);
    var product = productRepository.findById(productReviewDTO.getProductId())
                      .orElse(null);
    ...
    logger.info(String.format(
        "SAVED new product review: [Customer Email: %s] and [Product ID: %s]",
        productReviewDTO.getCustomerEmail(), productReviewDTO.getProductId()));
  ...
  protected ProductReviewDTO updateReview(ProductReviewDTO productReviewDTO) {
    var reviewExisting =
        repository.findById(productReviewDTO.getId())
            .orElseThrow(
                () -> new InvalidReviewException(INVALID_REVIEW_MESSAGE));
    ...
    logger.info(String.format("UPDATED product review with id: %s",
                              productReviewDTO.getId()));
  ...
  @Override
  public ProductReviewDTO getReviewOfCurrentUserIfFound(String productId,
                                                        String userEmail) {
    ProductReview existingReview =
        repository.getProductReviewByCurrentUser(productId, userEmail)
            .orElse(null);
    if (existingReview == null)
      ...
      return ProductReviewDTO.builder()
          ...
          .customerName(existingReview.getCustomer().getFirstName() +
                        existingReview.getCustomer().getLastName())
  ...
  public boolean isProductOrderedByCurrentUser(String productId, String email) {
    return productRepository.getAllOrderedProductIdByCustomerName(email)
        .contains(productId);
  }

  public Page<ProductReview> getAllReviewWithPagination(String productId) {
    return this.getAllReviewWithPagination(productId, 1, 1, "created_at",
                                           "desc");
  }

  private Page<ProductReview>
  getAllReviewWithPagination(String productId, int pageNo, int pageSize,
  ...
  private Pageable findPaginated(int pageNo, int pageSize, String sortField,
